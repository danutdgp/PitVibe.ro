...
function CommentCard({ comment, replies, postId, userId, nested = false }: { comment: CommentRecord; replies?: CommentRecord[]; postId: string; userId: string; nested?: boolean }) {
  ...
  return <article className={`rounded-2xl border border-white/8 p-4 ${nested ? "bg-white/[.025]" : "bg-white/[.035]"}`}>
    ...
    {!nested && <details className="group mt-3"><summary className="w-fit cursor-pointer list-none text-xs font-semibold text-violet-300">Răspunde</summary><form action={addComment} className="mt-3 flex gap-2"><input type="hidden" name="post_id" value={postId} /><input type="hidden" name="parent_id" value={comment.id} /><EmojiTextField name="comentariu" className="field min-h-10 py-2 pr-12 text-sm" maxLength={1000} required placeholder={`Răspunde-i lui ${author?.display_name ?? "acestui membru"}…`} /><button className="button-primary min-h-10 shrink-0 px-4 text-sm">Trimite</button></form></details>}
...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <section id="comentarii" className="mt-6">
      ...
      <form action={addComment} className="mt-4 flex gap-3"><input type="hidden" name="post_id" value={id} /><EmojiTextField name="comentariu" className="field pr-12" maxLength={1000} required placeholder="Scrie un comentariu…" /><button className="button-primary shrink-0" type="submit">Trimite</button></form>
