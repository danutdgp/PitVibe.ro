...
export async function AccountSuggestions() {
  ...
  const suggestions = (profiles ?? []).filter((profile) => !followedIds.has(profile.id)).sort((a, b) => dailyScore(a.id) - dailyScore(b.id)).slice(0, 5);
  ...
  return <section className="card mt-6 p-5">
    <div className="flex items-center justify-between"><h2 className="font-bold">Sugestii pentru tine</h2><Link href="/descopera" className="text-xs font-semibold text-violet-300 hover:text-violet-200">Vezi tot</Link></div>
